...
$diasParados7 = filter_input(INPUT_GET, 'diasParados7', FILTER_SANITIZE_NUMBER_INT);
$diasParados30 = filter_input(INPUT_GET, 'diasParados30', FILTER_SANITIZE_NUMBER_INT);

// print_r($diasParados30);
...
//query pra pegar a quantidade de todso os filtros ativos
$whereQtd = '
  a.fase_seq = 1
  ...
  $whereQtd .= '
    and exists (
        ...
        where ax.id_proj = p.id
          ...
          and DATEDIFF(IFNULL(ax.updated_at, NOW()), ax.created_at) >= '.$diasParados7.'
    )
 ';
} else if ($diasParados30) {
  $whereQtd .= '
    and exists (
        select 1 
        from avaliacoes ax
        where ax.id_proj = p.id
          and ax.fase_seq >= 1' . ($fase_seq ? ' AND ax.fase_seq = ' . $fase_seq : '') . '
          and DATEDIFF(IFNULL(ax.updated_at, NOW()), ax.created_at) >= '.$diasParados30.'
    )
 ';
}
...
if ($diasParados7) { 
 ...
 ';
  ...
  $whereFases .= ' and DATEDIFF(ifnull(a.updated_at, now()), a.created_at) >= '.$diasParados7.'';
} else if ($diasParados30) {
  $whereBase .= '
    and exists (
        select 1 
 ...
 ';
  ...
  $whereFases .= ' and DATEDIFF(ifnull(a.updated_at, now()), a.created_at) >= '.$diasParados30.'';
}
...
$qry = '
  ...
  order by p.id, a.fase_seq, dias desc
