...
$url = explode('/', $_SERVER['PHP_SELF']);
  // if(array_search('cadastrar_valida.php', $url)) {
  //   echo 'está no cadastro';
  // } else {
  //   echo 'não está no cadastro';
  // }
  // // print_r(array_search('cadastro.php'));
// print_r();
// echo '</pre>';
// exit;
...
<main class="container">
  <div class="d-flex justify-content-between align-items-center mt-3 mb-3">
    ...
    <span class="alert alert-info mb-0 py-2">
        IP de acesso: <strong><?= $ip ?></strong>
    </span>
  </div>  
      ...
      <form method="post" id="formprof">
        ...
        <div class="row">
          <div class="col-4">
            <div class="form-group">
              <label>Senha</label>
              <?php if(!array_search('cadastrar_valida.php', $url)): ?>
                <span class="badge badge-warning">Não mexa para não alterá-la</span>
              <?php endif ?>
              <input type="password" class="form-control" name="senha" maxlength="20" placeholder="">
            </div>
        </div>
        <hr>
