...
if ($cent_cood == 0) {
    $query = ' 
          ...
          select * from (
          select 
            p.id, concat(p.nome, " [Prof] - ", co.nome) nome,
            if(ca.dir_campus_id =  p.id, " SELECTED ", "") sel
          from 
            professores p 
            inner join colegiados co on co.id = p.id_colegiado
            inner join centros ce on ce.id = co.centro_id
            inner join campi ca on ce.campus_id = ca.id
          where  
             p.ativo = 1
             and ca.id = "'.$local.'"
          union all
          select 
             a.id, concat(a.nome, " [Agente]"), 
             if(ca.dir_campus_id =  a.id, " SELECTED ", "") sel
          from 
            agentes a
            inner join campi ca 
               on a.lotacao = ca.id and 
                  a.ativo = 1 and 
                  a.lotacao = "'.$local.'"
          ) a order by 3 desc, 2
           ';
