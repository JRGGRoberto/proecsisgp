...
// Filtro de status
$filtroStatus = filter_input(INPUT_GET, 'filtroStatus');
...
$vigenIni = filter_input(INPUT_GET, 'vigen_ini');
$vigenFim = filter_input(INPUT_GET, 'vigen_fim');
$tipoProj = filter_input(INPUT_GET, 'tipoProj', FILTER_SANITIZE_NUMBER_INT);// echo '<pre>';
// print_r($vigenIni);
// echo '</pre>';
...
//filtro de vigÊncia
if (!empty($vigenIni) && !empty($vigenFim)) {
    // período
    $condicoes[] = "(vigen_ini >= '{$vigenIni}' AND vigen_fim <= '{$vigenFim}')";
} elseif (!empty($vigenIni)) {
    // só início -> pega tudo que termina depois do início
    $condicoes[] = "(vigen_ini >= '{$vigenIni}' AND vigen_fim <= '2099-12-31' )";
} elseif (!empty($vigenFim)) {
    // só fim -> pega tudo que começa antes do fim
    $condicoes[] = "(vigen_ini >= '2020-01-01' AND vigen_fim <= '{$vigenFim}')";
}
...
if (!empty($condicoes)) {
    ...
    $query = '
    select 
        p.campus, p.protocolo, p.titulo, p.coord, te.nome tipo_exten,
        DATE_FORMAT(p.vigen_ini, "%d/%m/%Y") vigen_ini,
        DATE_FORMAT(p.vigen_fim, "%d/%m/%Y") vigen_fim,
    case  p.estado 
        when 0 then "Não submetido"
        when 1 then "Em avaliação"
        when 2 then "Não iniciado"
    ...
    // print_r('Não tem nenhum filtro ativo.');
    ...
    $qntdProjetos = ProjMaster::getQntdRegistros();

}

// print_r($qntdProjetos);
...
$projetos = Outros::qry($query);


//montar os where dinâmicos de acordo com os filtros ativos pelas condicoes
