document.addEventListener("DOMContentLoaded", () => {
  ...
  function render() {
    ...
    state.ranked.forEach((item, index) => {
      ...
      const btnsFP = document.createElement("div");
      const fimPrazo = document.createElement("button");
      fimPrazo.className = "btn btn-sm btn-outline-light text-left text-dark disabled";
      fimPrazo.innerHTML = '<strong>Prazo expirado</strong>';
      btnsFP.appendChild(fimPrazo);

      //Se passar da data de seleção, não deixa selecionar mais, não mostar botões
      if(item.sel_cand == 1) {
        div.append(row, btns);
      } else {
        div.append(row, btnsFP);
      }
